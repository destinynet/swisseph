...
//      goto iter_where;
  ...
  int swe_sol_eclipse_when_glob(double tjd_start, int ifl, int ifltype,
    ...
    DblObj dt1 = new DblObj(0);
    DblObj dt2 = new DblObj(0);
...
//next_try:
    while (true) {
      ...
      if ((retflag & SweConst.SE_ECL_TOTAL) != 0) {
        if ((retflag2 = eclipse_where(tret[0], SweConst.SE_SUN, null, ifl, geopos, dcore, serr)) == SweConst.ERR) {
          return retflag2;
        }
        dc[0] = dcore[0];
        if ((retflag2 = eclipse_where(tret[4], SweConst.SE_SUN, null, ifl, geopos, dcore, serr)) == SweConst.ERR) {
          return retflag2;
        }
        dc[1] = dcore[0];
        if ((retflag2 = eclipse_where(tret[5], SweConst.SE_SUN, null, ifl, geopos, dcore, serr)) == SweConst.ERR) {
          return retflag2;
        }
        dc[2] = dcore[0];
...
//end_search_global:
  ...
  private int eclipse_when_loc(double tjd_start, int ifl, double[] geopos,
    ...
    double M;
    double Mm;
    DblObj tjdr = new DblObj(0);
    DblObj tjds = new DblObj(0);
    DblObj dtint = new DblObj(0);
    double E;
    double Ff;
    double A1;
    ...
    while (true) {
      ...
      if (dctr.val > Math.abs(rsminusrm)) {/* partial, no 2nd and 3rd contact */
        ...
        for (m = 0, dt = tensec; m < 2; m++, dt /= 10) {
          for (j = 2; j <= 3; j++) {
            if (sw.swe_calc(tret[j], SweConst.SE_SUN, iflagcart | SweConst.SEFLG_SPEED, xs, serr) == SweConst.ERR) {
              return SweConst.ERR;
            }
...
//        goto next_lun_ecl;
   ...
   * @see destiny.swisseph.SweConst#OK
   * @see destiny.swisseph.SweConst#ERR
   * @see destiny.swisseph.SweConst#SEFLG_JPLEPH
   * @see destiny.swisseph.SweConst#SEFLG_SWIEPH
   * @see destiny.swisseph.SweConst#SEFLG_MOSEPH
   * @see destiny.swisseph.SweConst#SEFLG_TRUEPOS
   * @see destiny.swisseph.SweConst#SEFLG_HELCTR
   */
   ...
   * @see destiny.swisseph.SweConst#SEFLG_SWIEPH
   * @see destiny.swisseph.SweConst#SEFLG_MOSEPH
   * @see destiny.swisseph.SweConst#SE_CALC_RISE
   * @see destiny.swisseph.SweConst#SE_CALC_SET
   * @see destiny.swisseph.SweConst#SE_CALC_MTRANSIT
   * @see destiny.swisseph.SweConst#SE_CALC_ITRANSIT
   * @see destiny.swisseph.SweConst#SE_BIT_DISC_CENTER
   * @see destiny.swisseph.SweConst#SE_BIT_DISC_BOTTOM
   * @see destiny.swisseph.SweConst#SE_BIT_NO_REFRACTION
   * @see destiny.swisseph.SweConst#SE_BIT_FIXED_DISC_SIZE
   * @see destiny.swisseph.DblObj
   */
  ...
  int swe_nod_aps(double tjd_et, int ipl, int iflag,
    ...
    int iflJ2000;
    double plm;
    double t = (tjd_et - SwephData.J2000) / 36525;
    double dt;
    double[] x = new double[6];
    ...
    double[] xap;
    final int xndOffs = 6;
    final int xpeOffs = 12;
    final int xapOffs = 18;
    double incl;
    double sema;
    double ecce;
    double parg;
...
//      sm.swi_mean_lunar_elements(tjd_et, &xna[0], &xna[3], &xpe[0], &xpe[3]);
        DblObj xna0 = new DblObj(0.0);
        xna0.val = xna[0];
        DblObj xna3 = new DblObj(0.0);
        xna3.val = xna[3];
        DblObj xpe0 = new DblObj(0.0);
        xpe0.val = xpe[0 + xpeOffs];
        DblObj xpe3 = new DblObj(0.0);
   ...
   *               something fails.
   * @return SweConst.OK (0) or SweConst.ERR (-1)
   * @see Swecl#swe_nod_aps(double, int, int, int, double[], double[], double[], double[], java.lang.StringBuffer)
   * @see destiny.swisseph.SweConst#OK
   * @see destiny.swisseph.SweConst#ERR
   * @see destiny.swisseph.SweConst#SE_NODBIT_MEAN
   * @see destiny.swisseph.SweConst#SE_NODBIT_OSCU
   * @see destiny.swisseph.SweConst#SE_NODBIT_OSCU_BAR
   * @see destiny.swisseph.SweConst#SE_NODBIT_FOPOINT
   */
